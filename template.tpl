...
// REQUIRED: widgetId comes from the template field (not tag attributes)
var wid = data.widgetId || '';
if (!wid) {
  logToConsole('WelcomingWeb: Missing widgetId');
  if (data.gtmOnFailure) data.gtmOnFailure();
  return;
}

// Build the CDN URL with wid param so your bundle can read it
var url = 'https://cdn-01.welcomingweb.com/a11y-widget.bundle.js' + '?wid=' + encodeUriComponent(wid);
